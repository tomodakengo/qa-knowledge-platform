...
**Source issues**: #7（CI でローディングスピナーが消える前に注文確定ボタンを click していた）。その他は `gh issue list --label P-locator-timeout` で原典を辿れる
...
**Source issues**: #9（カート追加 API レスポンス前に total を assert していた。fixture 側で `waitForResponse` 待機が必要）
